// Admin API: Verify problem and send email notification to problem poster
...
export async function POST(request, { params }) {
  try {
    ...
    // Update problem: mark as verified/open and set notification_sent flag
    ...
    // Send email notification to problem poster
    try {
      await resend.emails.send({
        from: 'noreply@openpools.in',
        to: problem.email,
        bcc: 'ary7sharma@gmail.com',
        subject: `Your challenge is now live on Counterpools 🚀`,
        html: generateEmailHTML(problem),
      })

      console.log('Email sent successfully to', problem.email)
    } catch (emailError) {
      console.error('Failed to send email:', emailError)
      // Continue even if email fails - the problem is still verified
    }

    return new Response(
      JSON.stringify({ 
        success: true, 
        message: 'Problem verified and notification sent',
        data: updated[0]
      }),
    ...
    return new Response(
      JSON.stringify({ success: false, message: err.message || 'Failed to verify' }),
