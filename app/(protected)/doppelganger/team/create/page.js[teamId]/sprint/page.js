...
export default function SprintPage() {
        ...
        return (
            <div className="min-h-screen flex items-center justify-center">
                <div className="text-center">
                    ...
                    <p className="text-gray-500 font-black tracking-widest uppercase text-xs">SYNCHRONIZING COMMAND CENTER...</p>
                </div>
            </div>
        )
    }
    ...
    const isSprintActive = now >= sprintStart && now <= sprintEnd
    const sprintNotStarted = now < sprintStart
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
                    ...
                    <div className="lg:col-span-8 space-y-8">
                        {/* Problem Statement as Mission Parameters */}
                        <div className="relative">
                            <div className="absolute top-4 left-4 z-20">
                                <span className="px-2 py-1 bg-purple-500 text-white text-[8px] font-black uppercase tracking-widest rounded shadow-lg italic">MISSION_PARAMETERS</span>
                            </div>
                        ...
                        <div className="space-y-6">
                            ...
                            {logs.length === 0 ? (
                                <div className="glass-dark rounded-[2.5rem] p-16 border border-white/5 text-center">
                                    <p className="text-gray-600 font-bold uppercase tracking-[0.2em] italic text-xs">NO SIGNALS REGISTERED IN CURRENT STREAM</p>
                                </div>
                            ) : (
                                <div className="space-y-6">
                                    {logs.map((log, i) => (
                                        <div key={log.id} className="glass-dark rounded-[2.5rem] p-8 border border-white/5 hover:border-white/10 transition-all group animate-fadeInUp" style={{ animationDelay: `${0.1 * i}s` }}>
                                            <div className="flex items-center justify-between mb-6">
                                                <div className="flex items-center gap-4">
                                                    <div className="w-10 h-10 bg-white/5 rounded-xl flex items-center justify-center font-black text-xs text-purple-400 border border-white/5">
                                                        0{log.checkpoint_number}
                                                    </div>
                                                    <div>
                                                        <h3 className="font-black text-white italic tracking-tight uppercase">{log.title}</h3>
                    ...
                    <div className="lg:col-span-4 space-y-8 sticky top-24">
                        ...
                        <Link href={`/doppelganger/team/${teamId}/submit`}>
                            <button className="w-full py-6 bg-gradient-to-r from-green-500 to-emerald-500 text-white rounded-[2rem] font-black text-sm tracking-[0.3em] uppercase transition-all hover:scale-[1.02] active:scale-95 shadow-2xl shadow-green-500/20 flex items-center justify-center gap-3">
                        ...
                        <div className="glass-dark rounded-[2.5rem] p-8 border border-white/5">
                            <h3 className="text-[10px] font-black text-gray-500 uppercase tracking-[0.2em] mb-6">INTEL_BRIEF</h3>
                            <div className="space-y-4">
                                <div className="flex justify-between items-center text-[10px] font-bold">
                                    <span className="text-gray-600 uppercase">SIGNAL_RELIABILITY</span>
                                    <span className="text-purple-400">98.4%</span>
                                </div>
                                <div className="w-full h-1 bg-white/5 rounded-full overflow-hidden">
                                    <div className="w-[98.4%] h-full bg-purple-500"></div>
                                </div>
                                <p className="text-[9px] text-gray-600 font-bold uppercase tracking-tight leading-relaxed italic">
                                    Maintain high frequency signal logs to ensure maximum scoring potential. Signals are processed through GPT-4o for resonance analysis.
                                </p>
                            </div>
                        </div>
                    </div>
