...
export async function sendDNAInviteEmail(receiverEmail, receiverName, inviteUrl, dnaUrl) {
  try {
    const { data, error } = await resend.emails.send({
      from: 'OpenPools <hi@openpools.in>',
      to: receiverEmail,
      subject: 'Your Professional DNA is ready on OpenPools',
      html: `
        <!DOCTYPE html>
        <html>
          <head>
          ...
          <body style="margin: 0; padding: 0; background-color: #ffffff; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif;">
            <div style="max-width: 600px; margin: 0 auto; padding: 40px 20px;">
              ...
              <div style="margin-bottom: 30px;">
                ...
                <p style="color: #1f2937; margin: 0 0 24px 0; font-size: 18px; font-weight: 600;">
                  Your Professional DNA has been created!
                </p>

                <p style="color: #4b5563; margin: 0 0 24px 0; font-size: 16px; line-height: 1.6;">
                  We've analyzed your resume and created a unique DNA profile that showcases your skills, expertise, and professional identity. Your profile is ready and waiting for you to claim it.
                </p>

                <!-- DNA Preview Box -->
                <div style="background: linear-gradient(135deg, #6366f1 0%, #8b5cf6 100%); border-radius: 12px; padding: 24px; margin: 0 0 32px 0; text-align: center;">
                ...
                <ul style="color: #4b5563; margin: 0 0 32px 0; padding-left: 20px; font-size: 15px; line-height: 1.8;">
                  <li>Your unique DNA visualization based on your skills</li>
                  <li>AI-powered insights about your professional profile</li>
                  <li>Smart matching with complementary professionals</li>
                  <li>A shareable link to showcase your expertise</li>
                </ul>
                  ...
                  <table cellpadding="0" cellspacing="0" border="0">
                    <tr>
                      <td style="background-color: #6366f1; border-radius: 6px;">
                        <a href="${inviteUrl}" target="_blank" style="display: inline-block; background-color: #6366f1; color: #ffffff; padding: 14px 28px; text-decoration: none; border-radius: 6px; font-weight: 500; font-size: 16px; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif;">Claim Your DNA Profile</a>
                      </td>
                    </tr>
                  </table>
                </div>
                ...
                <p style="color: #6b7280; margin: 0 0 16px 0; font-size: 14px; line-height: 1.6;">
                  This invitation expires in 30 days. Simply sign up with this email address to claim your profile.
                </p>
    ...
    if (error) {
      console.error('Error sending DNA invite email:', error)
      return { success: false, error }
    }

    console.log('DNA invite email sent to:', receiverEmail)
    return { success: true, data }

  } catch (error) {
    console.error('Failed to send DNA invite email:', error)
    return { success: false, error: error.message }
  }
}
