...
// Soft skills that are not real technical/domain skills — filtered out globally
...
export function combineTeamKeywords(membersKeywords) {
  ...
  const keywords = Array.from(keywordMap.values())
    .filter(kw => !SOFT_SKILLS.has(kw.keyword.toLowerCase()))
    .map(kw => {
 ...
 */
export async function generateProblemStatement(combinedKeywords, perMember = []) {
...
- Do NOT generate the same challenge type for every team — analyze the skills and diverge`
  ...
  // Build keywords_used with guaranteed slots per member so everyone's skills appear
  const slotsPerMember = Math.ceil(15 / Math.max(perMember.length, 1))
  const seen = new Set()
  const keywords_used = []

  for (const memberKws of perMember) {
    const memberTop = memberKws
      .filter(kw => kw?.keyword && !SOFT_SKILLS.has(normalizeKeyword(kw.keyword).toLowerCase()))
      .sort((a, b) => (b.weight || 0) - (a.weight || 0))
      .slice(0, slotsPerMember)
      .map(kw => normalizeKeyword(kw.keyword))

    for (const kw of memberTop) {
      if (!seen.has(kw)) {
        seen.add(kw)
        keywords_used.push(kw)
      }
    }
  }
...
export async function getTeamCombinedKeywords(supabase, teamId) {
  const team = await getTeamWithDetails(supabase, teamId)
  if (!team) return { combined: [], perMember: [] }
  ...
  if (captainKeywords?.keywords) {
    ...
    perMember.push(captainKeywords.keywords)
  }
  ...
  for (const member of team.members.filter(m => m.invite_status === 'accepted' && m.user_id !== team.created_by)) {
    ...
    if (memberKeywords?.keywords) {
      ...
      perMember.push(memberKeywords.keywords)
    }
  }

  return { combined: combineTeamKeywords(allKeywords), perMember }
}
